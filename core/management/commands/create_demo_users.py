...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR('Les groupes n\'existent pas. Exécutez d\'abord setup_groups.')
            )
            return
        ...
        if not User.objects.filter(username='manager').exists():
            ...
            self.stdout.write(
                self.style.SUCCESS('Utilisateur "manager" créé (mot de passe: manager123)')
            )
        ...
        if not User.objects.filter(username='viewer').exists():
            ...
            self.stdout.write(
                self.style.SUCCESS('Utilisateur "viewer" créé (mot de passe: viewer123)')
            )
        ...
        if not User.objects.filter(username='superadmin').exists():
            ...
            self.stdout.write(
                self.style.SUCCESS('Utilisateur "superadmin" créé (mot de passe: admin123)')
            )
