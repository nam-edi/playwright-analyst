...
from testing.models import TestExecution, TestResult
    ...
    for project in projects:
        ...
        if success_rates:  # Seulement ajouter si il y a des données
            projects_data.append({
                'name': project.name,
                'data': success_rates,
                'color': generate_project_color(project.id)
            })
...
def user_toggle_active(request, user_id):
    ...
    if user.is_superuser and User.objects.filter(is_superuser=True, is_active=True).count() <= 1:
        return JsonResponse({
            'error': 'Impossible de désactiver le dernier superutilisateur actif.'
        }, status=400)
...
def update_tag_color(request):
    ...
    try:
        ...
        if Tag.objects.filter(project=tag.project, color=color).exclude(id=tag.id).exists():
            messages.error(request, f'Cette couleur est déjà utilisée pour un autre tag de ce projet.')
            return redirect('administration_dashboard') + '?section=tags'
...
def user_contexts_list(request):
    ...
    users_with_restricted_access = User.objects.filter(
        usercontext__isnull=False,
        groups__name__in=['Manager', 'Viewer']
    ).prefetch_related('groups', 'usercontext__projects').distinct()
