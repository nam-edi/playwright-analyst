...
from core.models import Test, TestResult, TestExecution, Project
import logging
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Optimisation terminée avec succès!')
        )
    ...
    def show_initial_stats(self):
        ...
        status_stats = TestResult.objects.values('status').annotate(
            count=Count('id')
        ).order_by('-count')
    ...
    def clean_orphaned_records(self):
        ...
        orphaned_results = TestResult.objects.filter(
            Q(test__isnull=True) | Q(execution__isnull=True)
        )
        ...
        if orphaned_count > 0:
            ...
            if not self.dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ {deleted_count} résultats orphelins supprimés')
                )
        else:
        ...
        unused_tests = Test.objects.annotate(
        ...
        if unused_count > 0:
            ...
            if not self.dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ {deleted_count} tests inutilisés supprimés')
                )
        else:
    ...
    def recalculate_statistics(self):
        ...
        if self.dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✓ {updated_count} exécutions mises à jour')
            )
    ...
    def optimize_tags(self):
        ...
        if unused_count > 0:
            ...
            if not self.dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'✓ {deleted_count} tags inutilisés supprimés')
                )
        else:
    ...
    def clean_old_data(self):
        ...
        if old_count > 0:
            self.stdout.write(
                f'Exécutions anciennes (>{self.days_to_keep} jours): {old_count}'
            )
            if not self.dry_run:
                # Les résultats seront supprimés en cascade
                deleted_count = old_executions.delete()[0]
                self.stdout.write(
                    self.style.SUCCESS(f'✓ {deleted_count} anciennes exécutions supprimées')
                )
        else:
    ...
    def show_final_stats(self):
        ...
        if total_results > 100000:
            self.stdout.write(
                self.style.WARNING(
                    '⚠️  Nombre élevé de résultats de tests. '
        ...
        if total_tests > 10000:
            self.stdout.write(
                self.style.WARNING(
                    '⚠️  Nombre élevé de tests. '
                    'Vérifiez les tests obsolètes.'
                )
            )
        
        avg_success_rate = TestExecution.objects.aggregate(
        ...
        if avg_success_rate and avg_success_rate < 80:
            self.stdout.write(
                self.style.ERROR(
                    f'⚠️  Taux de réussite moyen bas: {avg_success_rate:.1f}%. '
                    'Vérifiez la qualité des tests.'
                )
            )
        elif avg_success_rate:
            self.stdout.write(
                self.style.SUCCESS(
                    f'✓ Taux de réussite moyen: {avg_success_rate:.1f}%'
                )
            )